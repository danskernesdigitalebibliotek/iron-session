      ...
      save: {
        value: async function save() {
          ...
          if (sessionConfig.chunking?.enabled) {
            ...
            const testCookieValue = cookie.serialize(
            ...
            chunks.forEach((chunk, index) => {
              ...
              setCookie(
                res,
                cookie.serialize(chunkName, chunk, sessionConfig.cookieOptions)
              );
            });
            setCookie(
              res,
              cookie.serialize(sessionConfig.cookieName, "", {
                ...sessionConfig.cookieOptions,
                maxAge: 0
              })
            );
...
async function getIronSessionFromCookieStore(cookieStore, sessionOptions, sealData2, unsealData2) {
  ...
  let sessionConfig = getSessionConfig(sessionOptions);
  const sealFromCookies = reconstructCookieWithStore(
    cookieStore,
    sessionConfig.cookieName
  );
  const session = sealFromCookies ? await unsealData2(sealFromCookies, {
